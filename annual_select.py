...
#         利回り×2.5（+0.5）、配当性向×2.0（+0.5）で利回り・持続性を強化
#         安定性・ROE・成長・財務は据え置き
# 改良10: Isolation Forestを業種別相対評価に変更
#         全銘柄同一モデル → 業種グループ内モデル（MIN_SECTOR_SAMPLES=10）
#         サンプル不足業種は全体モデルにフォールバック
#         電力・金融・不動産等の業種特有財務構造による誤検知を低減
# ==============================
...
# ============================================================
# 【追加・改良10】Isolation Forest による異常検知（業種別相対評価）
# 対象：データ品質「正常」銘柄のみ
# 出力列：anomaly_flag（-1=異常, 1=正常）、anomaly_score（低いほど異常度高）
#         anomaly_reason（スコアが低い主因列を列挙）
#
# 【改良10の変更点】
# 旧：全銘柄を同一のIsolation Forestで評価
#     → 電力・金融・不動産など業種特有の財務構造が「異常」と誤判定されやすい
# 新：業種グループ内でIsolation Forestを実行（業種内相対評価）
#     → 同業種内での外れ値のみを異常判定するため業種特性を考慮できる
#     サンプル数が MIN_SECTOR_SAMPLES 未満の業種は全体モデルのスコアを使用
# ============================================================
MIN_SECTOR_SAMPLES = 10  # 業種別モデルを使う最小サンプル数

def _run_isolation_forest(X_raw: pd.DataFrame, contamination: float):
    """
    RobustScaler + IsolationForest を実行し (flags, scores, X_scaled_df) を返す。
    """
    use_cols = [c for c in X_raw.columns if X_raw[c].notna().sum() > 0]
    X_filled = X_raw[use_cols].fillna(0)
    scaler   = RobustScaler()
    ...
    X_scaled_df = pd.DataFrame(X_scaled, index=X_raw.index, columns=use_cols)
    return flags, scores, X_scaled_df
...
def detect_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── 全体モデル（フォールバック用） ──────────────────────────
    global_flags, global_scores, global_scaled_df = _run_isolation_forest(
        target[use_cols], ANOMALY_CONTAMINATION
    )
    # まず全体モデルの結果を仮セット
    df.loc[target.index, "anomaly_flag"]  = global_flags
    df.loc[target.index, "anomaly_score"] = np.round(global_scores, 4)

    # ── 業種別モデル ────────────────────────────────────────────
    sector_counts = target["業種"].value_counts()
    large_sectors = sector_counts[sector_counts >= MIN_SECTOR_SAMPLES].index.tolist()
    small_sectors = sector_counts[sector_counts <  MIN_SECTOR_SAMPLES].index.tolist()

    covered_idx = []  # 業種別モデルで処理済みのindex

    for sector in large_sectors:
        grp = target[target["業種"] == sector]
        if len(grp) < MIN_SECTOR_SAMPLES:
            continue
        s_flags, s_scores, s_scaled_df = _run_isolation_forest(
            grp[use_cols], ANOMALY_CONTAMINATION
        )
        df.loc[grp.index, "anomaly_flag"]  = s_flags
        df.loc[grp.index, "anomaly_score"] = np.round(s_scores, 4)
        covered_idx.extend(grp.index.tolist())

        # anomaly_reason を業種別スケール基準で設定
        anomaly_grp_idx = grp.index[s_flags == -1]
        for idx in anomaly_grp_idx:
            row_abs = s_scaled_df.loc[idx].abs()
            top = row_abs.nlargest(3).index.tolist()
            df.loc[idx, "anomaly_reason"] = " / ".join(top)

    # ── 小業種はグローバルモデルのanomaly_reasonのみ設定 ────────
    small_idx = target[target["業種"].isin(small_sectors)].index
    anomaly_small_idx = [i for i in small_idx if global_flags[target.index.get_loc(i)] == -1]
    for idx in anomaly_small_idx:
        row_abs = global_scaled_df.loc[idx].abs()
        top = row_abs.nlargest(3).index.tolist()
        df.loc[idx, "anomaly_reason"] = " / ".join(top)

    n_anomaly = int((df.loc[target.index, "anomaly_flag"] == -1).sum())
    print(f"✅  異常検知完了: {n_anomaly} 銘柄を異常判定 (全{len(target)}社中)")
    print(f"   業種別モデル適用: {len(large_sectors)} 業種 / "
          f"全体モデルフォールバック: {len(small_sectors)} 業種")
    return df
