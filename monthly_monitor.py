...
# ==============================
# 【追加1・改良10】月次 Isolation Forest スコアリング（業種別相対評価）
#
# 設計：
#   AllCandidates（annual全候補）を背景母集団として業種グループを構築
#   Final7の各銘柄を、同業種の背景データと合わせて業種内相対評価
#   業種サンプルが MIN_SECTOR_SAMPLES 未満の場合は全体モデルにフォールバック
# ==============================
...
if monthly_feature_rows:
    ...
    # Final7の業種情報をdetailsから取得
    ticker_sector = {t: details[t].get("業種", "") for t in monthly_tickers}
    ...
    if not background_df.empty:
        ...
        combined_global = df_monthly[ANOMALY_FEATURES_MONTHLY].fillna(0)

    use_cols_global = [c for c in ANOMALY_FEATURES_MONTHLY if c in combined_global.columns]
    X_global = combined_global[use_cols_global].fillna(0)
    scaler_g = RobustScaler()
    X_global_scaled = scaler_g.fit_transform(X_global)
    iso_global = IsolationForest(n_estimators=300, contamination=0.05, random_state=42, n_jobs=-1)
    iso_global.fit(X_global_scaled)
    n = len(df_monthly)
    global_scores_m = iso_global.score_samples(X_global_scaled[-n:])
    global_flags_m  = iso_global.predict(X_global_scaled[-n:])

    # まずグローバルスコアをセット
    for i, t in enumerate(monthly_tickers):
        for r in log_rows:
            if r["ティッカー"] == t:
                r["monthly_anomaly_score"] = round(float(global_scores_m[i]), 4)
                r["monthly_anomaly_flag"]  = int(global_flags_m[i])
                break

    # ── 業種別モデル（背景データがある場合のみ） ────────────────
    sector_model_applied = []
    sector_fallback      = []

    if not background_df.empty and "業種" in background_df.columns:
        for t in monthly_tickers:
            sector = ticker_sector.get(t, "")
            if not sector:
                sector_fallback.append(t)
                continue

            bg_sector = background_df[background_df["業種"] == sector]
            if len(bg_sector) < MIN_SECTOR_SAMPLES:
                sector_fallback.append(t)
                continue
            ...
            use_cols_s = [c for c in ANOMALY_FEATURES_MONTHLY if c in combined_s.columns]
            X_s = combined_s[use_cols_s].fillna(0)
            scaler_s = RobustScaler()
            X_s_scaled = scaler_s.fit_transform(X_s)

            iso_s = IsolationForest(n_estimators=300, contamination=0.05, random_state=42, n_jobs=-1)
            iso_s.fit(X_s_scaled)

            # 末尾1行（当該銘柄）のスコアを取得
            score_s = iso_s.score_samples(X_s_scaled[[-1]])[0]
            flag_s  = iso_s.predict(X_s_scaled[[-1]])[0]

            for r in log_rows:
                if r["ティッカー"] == t:
                    r["monthly_anomaly_score"] = round(float(score_s), 4)
                    r["monthly_anomaly_flag"]  = int(flag_s)
                    break
            sector_model_applied.append(f"{t}({sector})")
    else:
        sector_fallback = monthly_tickers[:]

    print(f"✅  月次異常スコア計算完了")
    if sector_model_applied:
        print(f"   業種別モデル適用: {', '.join(sector_model_applied)}")
    if sector_fallback:
        print(f"   全体モデルフォールバック: {', '.join(sector_fallback)}")
