...
#### 【改良10】業種別相対評価（2026-06 実装）

旧実装では全銘柄を同一のIsolation Forestで評価していたため、  
電力・金融・不動産・自動車など業種特有の財務構造（高負債比率・低ROE等）が  
「異常」として誤判定される問題があった。

改良後の動作：
```
1. 業種グループ内でIsolation Forestを実行（業種内相対評価）
2. サンプル数 < MIN_SECTOR_SAMPLES（10件）の業種は
   全体モデルのスコアをフォールバックとして使用
3. annual・monthlyとも同じ設計を適用
```
...
### ~~1. 業種別平均指標による相対評価~~ ✅ 完了（2026-06 改良10）
業種別Isolation Forestによる相対評価を実装済み。  
`MIN_SECTOR_SAMPLES=10` 未満の業種は全体モデルをフォールバックとして使用。
...
**背景：** 業種別評価への切り替えによりスコア分布が変化する可能性がある。  
**方針：** 2026年6月・7月のmonthly結果を確認後、必要に応じて`monthly_monitor.py`の`ANOMALY_SCORE_THRESH`を調整する。
